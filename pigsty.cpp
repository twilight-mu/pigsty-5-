...
//		break;
		...
		cout << "Please enter a character('0'-quit, 'd'-day, 'm'-month): ";
		cin>>s;
		if (s == '0') break;	// If enterd `0`, break the loop
		if(s=='d')
		{
			oneday(ps);
			day+=1;
			cout << "After one day..." << endl;
		}
		if(s=='m')
		{
			for(int i=0;i<30;i++)
		...
		if(day>=90)
		{
			...
			cout<<"已满三个月将自动出圈一批猪！"<<endl;
			sellpig2=getallpig(ps);
			cout<<"出圈成功，共计出圈"<<sellpig1-sellpig2<<"只猪,"<<"得到"<<sumprice<<"元！"<<endl; 
			cout<<"已满三个月将自动入圈一批猪！"<<endl;
			buypig(ps);
			cout<<"入圈成功！"<<endl;
